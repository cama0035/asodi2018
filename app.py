...
line_chart = px.line(df_mask,
                   ...
                   title='<b>Cantidad de Pacientes por año<b>',
                   color_discrete_sequence =['#F63366', '#2e29cc', '#20a825']*len(df_mask),
                   template= 'plotly_white')
#st.plotly_chart(line_chart)
line_chart.update_yaxes(title_text="Cantidad")


# --- PLOT SCATTER CHART
scatter_chart = px.scatter(df_mask,
                   x='ANOS',
                   y=['HOSPITALES', 'CENTROS', 'CASAS'],
                   title='<b>Distribución Porcentual de Pacientes por año<b>',
                   color_discrete_sequence =['#F63366', '#2e29cc', '#20a825']*len(df_mask),
                   template= 'plotly_white')
#st.plotly_chart(scatter_chart)
scatter_chart.update_yaxes(title_text="Porcentaje")


# --- DISPLAY LINE & SCATTER CHARTS
col1, col2 = st.columns(2)
col1.plotly_chart(line_chart, use_container_width=True)
col2.plotly_chart(scatter_chart, use_container_width=True)
